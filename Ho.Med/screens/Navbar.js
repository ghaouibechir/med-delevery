...
class Navbar extends Component {
  ...
  fetchParasData = async () => {
    try {
      let response = await axios.get("http://192.168.1.20:5000/para/paras");
      this.setState({medecine:response.data});
    } catch (error) {
      console.log(error);
    }
  ...
  myCart(id) {
    ...
    axios
      .put(`http://192.168.1.20:5000/OrderId/${this.state.id}`, { id })
      .then((res) => {
            ...
            <View style={{ flexDirection: "row" }}>
              <TouchableOpacity>
                <Fontisto
                  ...
                  style={{ marginLeft: Sizes.fixPadding + 120.0 }}
